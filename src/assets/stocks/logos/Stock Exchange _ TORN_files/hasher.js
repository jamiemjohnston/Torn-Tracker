...
function _getWindowHash(){var result=_hashValRegexp.exec(hasher.getURL());return(result&&result[1])?decodeURIComponent(result[1]):'';}
...
return path;}
hasher={VERSION:'1.1.4',appendHash:'',prependHash:'/',separator:'/',changed:new Signal(),stopped:new Signal(),initialized:new Signal(),init:function(){if(_isActive)return;_hash=_getWindowHash();if(_isHashChangeSupported){_addListener(window,'hashchange',_checkHistory);}else{if(_isLegacyIE){if(!_frame){_createFrame();}
_updateFrame();}
